class ActionWithTarget <Action
  ...
  def execute_with_target!(target)
    ...
    card.flags[:target] << {action:self , target:target , description: "#{card.name} #{self.name.downcase}s #{target.name}"}
    target.flags[:target_by] = [] if target.flags[:target_by] == nil
    target.flags[:target_by] << {action:self , target:card , description: "#{card.name} #{self.name.downcase}s #{target.name}"}
  end
